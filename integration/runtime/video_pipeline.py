...
class VideoPipeline:
    def __init__(
        ...
        # Optional actual source FPS to use in TrackerWrapper, fallback to 30 if unavailable
        source_fps = 30
        ...
        # Validated Tracker defaults: minimum_iou_threshold=0.1, minimum_consecutive_frames=2
        ...
        if self.output_path:
            ...
            # Determine size. We'll use 1280x720 max bounds for display/output
            width = int(self.source._cap.get(cv2.CAP_PROP_FRAME_WIDTH))
            height = int(self.source._cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
            if width == 0 or height == 0:
                width, height = 1280, 720
            
            # Scale down for output/preview while preserving aspect ratio
    ...
    def run(self):
        ...
        while True:
            ...
            # KEY REQUIREMENT: Person-Only Tracking Boundary
            # Only detections where class_name == "person" are sent to Member 2 ByteTrack
            ...
            # Process zones and events for tracked people
            for s in states:
                ...
                for e in evs:
                    ...
                    if event_json["event_type"] == "restricted_zone_intrusion" and self.backend_url:
                        try:
                            # POST to backend using canonical output
                            resp = requests.post(f"{self.backend_url}/api/events", json=event_json)
                            if resp.status_code not in (200, 201):
                                logger.error(f"Backend POST failed: {resp.status_code} {resp.text}")
                            else:
                                logger.info(f"Event {event_json['event_id']} posted successfully.")
                        except Exception as ex:
                            logger.error(f"Failed to post event: {ex}")
            
            self.frames_processed += 1
            
            if self.display or self.out_writer:
                self._render_preview(frame.frame, person_dets, other_dets, states, inf_elapsed, start_time)
        ...
        if self.frames_processed > 0:
            logger.info(f"Average inference: {(self.total_inference_time / self.frames_processed) * 1000:.2f} ms")
    ...
    def _render_preview(self, frame_bgr, person_dets, other_dets, states, inf_elapsed, pipeline_start_time):
        # Resize to max 1280x720 for display, preserving aspect ratio
        ...
        # Draw Person Tracks (from TrackStateManager so we get confirmed IDs and footpoints)
        # We use tracks from state_mgr instead of just tracker because states are the logical entities
        ...
        # Draw other non-person detections (no tracking ID)
        for d in other_dets:
